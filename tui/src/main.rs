                    ...
                    Ok(mut child) => {
                        ...
                        loop {
                            if tokio::time::Instant::now() >= deadline {
                                tracing::warn!("Gateway did not become ready within 30s, continuing anyway");
                                break;
                            }
                            ...
                            match child.try_wait() {
                                Ok(Some(status)) => {
                                    ...
                                    eprintln!("Gateway exited with {status}. Check that 'smg' is the Rust binary (not Python).");
                                    eprintln!("Install with: cargo install --path model_gateway");
