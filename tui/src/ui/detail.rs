...
fn render_config(f: &mut Frame, worker: &WorkerInfo, area: Rect) {
    ...
    let health_color = if worker.is_healthy { theme::GREEN } else { theme::RED };
    let health_text = if worker.is_healthy { "healthy" } else { "unhealthy" };

    let short_url = worker.url
...
fn render_stats(
    ...
    lines.push(line_kv("Req/s", &format!("{:.1}", worker_rps)));

    // Circuit breaker (all workers)
    let cb_label = if worker.is_healthy { "closed" } else { "open" };
    let cb_color = if worker.is_healthy { theme::GREEN } else { theme::RED };
    lines.push(Line::from(vec![
        Span::styled("Circuit: ".to_string(), Style::default().fg(theme::TEXT_MUTED)),
        ...
        if let Some(wl) = worker_load {
            if let Some(ref details) = wl.details {
                ...
                for snap in &details.loads {
                    ...
                    lines.push(line_kv("Running", &format!(
                        "{} / {} max",
                    ...
                    lines.push(line_kv("KV Cache", &format!(
                        "{} / {} ({:.1}%)",
                        snap.num_used_tokens, snap.max_total_num_tokens,
                    ...
                    if snap.gen_throughput > 0.0 {
                        lines.push(line_kv("Gen Throughput", &format!("{:.1} tok/s", snap.gen_throughput)));
                    }
                }
            }
        }
