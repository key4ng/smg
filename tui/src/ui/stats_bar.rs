...
pub fn render_stats_bar(f: &mut Frame, state: &GatewayState, area: Rect) {
    ...
    if width < 80 {
        // Ultra-compact: just logo + status
        ...
        let line = Line::from(vec![Span::styled("⎔ SMG ", theme::title()), conn]);
        f.render_widget(Paragraph::new(line).style(bg), area);
        return;
    }

    if width < 100 {
        // Compact: logo + workers only (2 columns)
        ...
        let logo = Line::from(vec![Span::styled("⎔ SMG ", theme::title()), conn]);
